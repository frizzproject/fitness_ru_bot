...
async def processing_menu(msg: types.Message):
    ...
    if ( msg.text == '🍩 Рассчитать каллории' ):
        try:
            await FORM_DATA.age.set()
            await bot.send_message(msg.from_user.id,'<i>Отлично! Ты выбрал(а) функцию <b>расчёта каллоиий</b>. Теперь мне нужна будет некоторая информация про тебя. Если ты вдруг введёшь неправильные данные, то можешь отменить\nдействие командой: /cancel.</i>', reply_markup = kb_menu, parse_mode = 'HTML')
