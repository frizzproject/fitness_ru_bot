...
from keyboards import kb_menu, kb_gender, kb_active, kb_formuls, kb_share
from untils import *
...
async def calories_processing(msg: types.Message):
    try:
        ...
        await msg.answer('<i>Отлично! Ты выбрал(а) функцию <b>расчёта каллоиий</b>. Теперь мне нужна будет некоторая информация про тебя. Если ты вдруг введёшь неправильные данные, то можешь отменить действие командой: /cancel.</i>', parse_mode = 'HTML')
...
async def processing_menu(msg: types.Message):
    ...
    if ( msg.text == '🍩 Каллории' ):
        try:
            await FORM_DATA.age.set()
            await bot.send_message(msg.from_user.id,'<i>Отлично! Ты выбрал(а) функцию <b>расчёта каллоиий</b>. Теперь мне нужна будет некоторая информация про тебя. Если ты вдруг введёшь неправильные данные, то можешь отменить действие\nкомандой: /cancel.</i>', reply_markup = kb_menu, parse_mode = 'HTML')
        ...
        try:
            ...
            await bot.send_message(msg.from_user.id,'<i>Отлично! Ты выбрал(а) функцию <b>расчёта индекса массы тела</b>. Теперь мне нужна будет некоторая информация про тебя. Если ты вдруг введёшь неправильные данные, то можешь отменить действие\nкомандой: /cancel.</i>', reply_markup = kb_menu, parse_mode = 'HTML')
